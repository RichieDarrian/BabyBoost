...
def save_child_profile_picture(file, child_id):
    ...
    if IS_VERCEL:
        ...
        upload_path = Path(app.static_folder) / "uploads" / "children"
        upload_path.mkdir(parents=True, exist_ok=True)
        for old_file in upload_path.glob(f"{child_id}.*"):
            old_file.unlink(missing_ok=True)
        file.save(upload_path / filename)
        return f"uploads/children/{filename}"
...
def save_user_profile_picture(file, email):
    ...
    if IS_VERCEL:
        ...
        upload_path = Path(app.static_folder) / "uploads" / "users"
        upload_path.mkdir(parents=True, exist_ok=True)
...
def delete_picture_if_exists(url_or_path):
    if not url_or_path:
        return
    if IS_VERCEL:
        try:
            vercel_blob.delete(url_or_path, {"token": BLOB_TOKEN})
        except Exception:
            pass
    else:
        try:
            file_path = Path(app.static_folder) / url_or_path
            if file_path.exists():
                file_path.unlink()
        except Exception:
            pass
...
def build_new_child(name, birthdate, height, weight, gender, existing):
    ...
    return {
        "id": child_id,
        "profile_picture": "img/default-children-picture.svg",
        "name": name,
...
def add_child():
    ...
    picture_path = save_child_profile_picture(file, child["id"])
    if picture_path:
...
def delete_child(child_id):
    ...
    child = children.get(child_id)
    if child:
        delete_picture_if_exists(child.get("profile_picture"))
...
def api_predict():
    payload = request.get_json(silent=True) or {}
    gender_s = str(payload.get("gender", ""))
...
def delete_account():
    ...
    user = users.get(email)
    if user:
        delete_picture_if_exists(user.get("profile_picture"))
