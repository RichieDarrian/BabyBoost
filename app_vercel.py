...
# ---- File helpers (Vercel Blob) ----

def allowed_file(filename):
    return "." in filename and filename.rsplit(".", 1)[1].lower() in ALLOWED_EXTENSIONS


def delete_blob_if_exists(url):
    """Safely delete a blob by its URL. Silently ignores errors."""
    if not url:
        return
    try:
...
def save_profile_picture(file, child_id):
    """Upload a child's profile picture to Vercel Blob. Returns the public URL."""
...
def save_user_profile_picture(file, email):
    """Upload a user's profile picture to Vercel Blob. Returns the public URL."""
    if not file or file.filename == "":
        return None
    if not allowed_file(file.filename):
        return None
...
def add_child():
    ...
    if picture_url:
        child["profile_picture"] = picture_url
...
def update_profile():
    ...
    session["user_name"] = first_name
    if picture_url:
        users[new_email]["profile_picture"] = picture_url
